...
export default function QuizSolvePage() {
  ...
  const [attempt, setAttempt] = useState<AttemptWithAnswersDto | null>(null);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        await saveAnswers(attempt.id, answersArray);
      } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await submitAttempt(attempt.id, answersArray);
      router.push(`/student/quiz/result/${attempt.id}`);
    } catch (err) {
